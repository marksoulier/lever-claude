    ...
    async (server) => {
      ...
      server.tool(
        ...
        async ({
          ...
          // Sanity check: flag implausibly large balances before writing to DB.
          // Common LLM parsing error: "$80,000" gets passed as 8000000 instead of 80000.
          if (current_balance > 0 && annual_income && current_balance > annual_income * 50) {
            return { content: [{ type: "text" as const, text: `current_balance of $${current_balance.toLocaleString()} appears to be a data entry error — it's ${Math.round(current_balance / annual_income)}× the annual income of $${annual_income.toLocaleString()}. Please confirm the exact dollar amount with the user (e.g. "$80,000" should be passed as 80000, not 8000000).` }] };
          }
